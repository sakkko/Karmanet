...
pthread_mutex_t upload_list_mutex = PTHREAD_MUTEX_INITIALIZER;
...
void uploader_func(void *args) {
	...
	while (1) {
		...
		if ((rc = pthread_mutex_lock(&upload_list_mutex)) != 0) {
			fprintf(stderr, "uploader_func error - can't acquire lock: %s\n", strerror(rc));
			pthread_exit((void *)-1);
		}

		ulnode = get_upload();
		
		if ((rc = pthread_mutex_unlock(&upload_list_mutex)) != 0) {
		...
		if ((i = recv_packet_tcp(ulnode->socksd, &recv_pck)) < 0) {
			fprintf(stderr, "uploader_func errror - recv_packet_tcp failed\n");
			if (close_sock(ulnode->socksd) < 0) {
				perror("uploader_func error - close_sock failed");
			}
			if (write_err(ulinfo->th_pipe, ulinfo->pipe_mutex, "GET") < 0) {
		...
		if ((i = get_filepath(buf, ulnode->file_info.filename, (const unsigned char *)recv_pck.data)) < 0) {
		...
		get_file_info(fd, ulnode);
		send_pck.data_len = create_infofile_str(send_pck.data, (const unsigned char *)recv_pck.data, ulnode);
		memcpy(ulnode->file_info.md5, recv_pck.data, MD5_DIGEST_LENGTH);
		print_file_info(&ulnode->file_info);
		...
		while ((i = recv_packet_tcp(ulnode->socksd, &recv_pck)) > 0) {
			if (strncmp(recv_pck.cmd, CMD_GET, CMD_STR_LEN)) {
			...
			if (send_chunk(fd, recv_pck.index, ulnode) < 0) {
...
int add_upload(int socksd) {
	...
	if ((rc = pthread_mutex_lock(&upload_list_mutex)) != 0) {
		fprintf(stderr, "add_upload error - can't acquire lock: %s\n", strerror(rc));
		return -1;
	}

	insert_upload(socksd);

	if ((rc = pthread_mutex_unlock(&upload_list_mutex)) != 0) {
		fprintf(stderr, "add_upload error - can't release lock: %s\n", strerror(rc));
		return -1;
	}
...
int uploader_run(struct uploader_info *ulinfo) {
	...
	if ((rc = pthread_create(&ulinfo->thread, NULL, (void *)uploader_func, (void *)ulinfo)) != 0) {
